...
-- 测试模式
local test_mode = GetLockConfig("test_mode")
-- 权限模式
local permission_mode = GetLockConfig("permission_mode")
-- 管理员是否不受权限控制
local admin_option = GetLockConfig("admin_option")
-- 防止怪物摧毁建筑
local cant_destroyby_monster = GetLockConfig("cant_destroyby_monster")
-- 农作物防挖范围
local firesuppressor_dig = GetLockConfig("firesuppressor_dig")
--远古犀牛是否允许拆家
local minotaur_destroy = GetLockConfig("minotaur_destroy")
-- 保存权限的物品
        ...
        if inst.components.named ~= nil then
            if userid ~= nil then
                ...
                if ownerName ~= nil then
                    ...
                    --TheNet:Announce(inst.name .. "的所有者为" .. ownerName) 
            ...
            if item.gd_superlevel == nil then
                ...
                elseif cant_destroyby_monster and walls_state_config.walls_normal[itemPrefab] == nil then
                    ...
                    elseif config_item.winter_trees_table[itemPrefab] then
                        --这里是把东西设为不工作，而薇诺娜聚光灯被设为"CantWorkable"的时候会被加上"Noclick",所以这里把探照灯区别对待;沃姆伍德种在地上的种子如果CantWorkable就不能挖了。
                        -- 树无敌
                        makeWinterTreeCantAttack(item)
                    elseif
                        item.components.workable ~= nil and
                            not (item:HasTag("spotlight") or item:HasTag("notreadyforharvest"))
                     then
                        local workAction =
                            item.components.workable:CanBeWorked() and item.components.workable:GetWorkAction()
                ...
                --					elseif item:HasTag("spotlight") then
                end
            end
        else
            AddBurnable(item)
...
-- 设置物体权限
GLOBAL.SetItemPermission = function(item, player, forer)
    -- 处理mod物品等
...
local SetItemPermission = GLOBAL.SetItemPermission
...
function makeDeployCantAttack(inst)
    ...
    if inst.components.workable ~= nil then
        ...
        --旧防熊锁没添加“numworks”这一参数，大概是游戏更新了，现已添加
        local function onwork(inst, worker, workleft, numworks)
            ...
            -- 有权限时直接处理..主机或管理员直接可攻击                     玩家发起的攻击且墙体不无敌                                            怪物发起的攻击且墙体不无敌                                                                  犀牛发起的攻击
            if
                CheckItemPermission(leader, inst, true) or
                    ((leader:HasTag("player") and CheckWallActionPermission(inst.prefab, 1)) or
                        (leader:HasTag("player") == false and CheckWallActionPermission(inst.prefab, 2))) or
                    (minotaur_destroy and worker.prefab == "minotaur")
             then
                ...
                if inst.avghealth == nil or inst.components.health == nil or inst.components.health.currenthealth == 0 then
                    ...
                    if inst.components.workable.old_onwork ~= nil then
                        --旧防熊锁没添加“numworks”这一参数，大概是游戏更新了，现已添加
                        inst.components.workable.old_onwork(inst, worker, workleft, numworks)
                    end
                else
                    ...
                    inst.components.health.old_ondelta(
                        inst,
                        inst.currenthealth / inst.components.health.maxhealth,
                        inst.components.health:GetPercent()
                    )
        ...
        if inst.components["health"] and inst.components.health.ondelta then
            ...
            inst.components.health.ondelta = function(inst, old_percent, new_percent)
                ...
                -- 无条件允许修补
                if
                    inst.components.health.currenthealth > inst.currenthealth or
                        ((leader ~= nil and leader:HasTag("player") and CheckWallActionPermission(inst.prefab, 1)) or
                            ((leader == nil or leader:HasTag("player") == false) and
                                CheckWallActionPermission(inst.prefab, 2)))
                 then
                    inst.currenthealth = inst.components.health.currenthealth
