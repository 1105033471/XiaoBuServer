...
AddPrefabPostInit("rock_avocado_fruit",function(inst)
	...
	local old_onaccept = inst.components.workable.onwork 

	local function onaccept(inst, miner, workleft, workdone)

		local pos = miner:GetPosition()

		old_onaccept(inst, miner, workleft, workdone) 

		_G.TheWorld:DoTaskInTime(0.5,function(inst_1)
			local x, y, z = pos:Get()
			local ents = TheSim:FindEntities(x, y, z, SEE_ITEM_STACK_DIST, { "_inventoryitem" },{ "INLIMBO", "NOCLICK", "catchable", "fire" })
			for _, objBase in pairs(ents) do
				if objBase:IsValid() and objBase.components.stackable and not objBase.components.stackable:IsFull() then
	...
	inst.components.workable:SetOnWorkCallback(onaccept)
end)