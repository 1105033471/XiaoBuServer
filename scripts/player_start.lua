-- 给予玩家初始物品
local give_start_item = GetLockConfig("give_start_item")
local give_start_trinket = GetLockConfig("give_start_trinket")
...
local function StartingInventory(inst, player)
    ...
    giveItemToPlayer(startInventory, 1, "armorwood") --给1个木甲
	if give_start_trinket then
    giveItemToPlayer(startInventory, 1, "trinket_15") --给白色主教
    giveItemToPlayer(startInventory, 1, "trinket_28") --给白色战车
    giveItemToPlayer(startInventory, 1, "trinket_30") --给白色骑士
    giveItemToPlayer(startInventory, 8, "fossil_piece") --给化石碎片
    end
    --初始进入的时间是冬天或者临近冬天的时候
    if
    ...
    if
     ...
     then
		giveItemToPlayer(startInventory, 8, "nitre") --硝石
		giveItemToPlayer(startInventory, 8, "ice") --冰
		giveItemToPlayer(startInventory, 1, "heatrock") --热能石
		giveItemToPlayer(startInventory, 1, "watermelonhat") --西瓜帽
    end
