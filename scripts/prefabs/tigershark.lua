...
local prefabs =
{
	"tigersharkshadow",     -- todo
    "groundpound_fx",
    "groundpoundring_fx",   -- todo
	"shark_gills",
}
...
local function GoHomeAction(inst)
    ...
    return homePos ~= nil and DoAction(inst, nil, ACTIONS.WALKTO, nil, homePos) or nil
end
...
local function RetargetFn(inst)
    ...
    if  not inst:HasTag("aquatic") and
        not inst.sg:HasStateTag("specialattack") and
    ...
    -- 把水变成陆地
    if  inst:HasTag("aquatic") and not
        inst.sg:HasStateTag("specialattack") and
    ...
    -- 湖里不会攻击房子 dentro do lago nao ataca a casa
...
--	inst.SoundEmitter:PlaySound("volcano/creatures/crocodog/emerge")

	if onwater and not inst.sg:HasStateTag("specialattack") then
        MakeWater(inst) 
	end
    
	if not onwater and not inst.sg:HasStateTag("specialattack") and not casaperto then
        MakeGround(inst)
	end	
