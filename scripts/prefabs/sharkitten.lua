...
local function OnAttacked(inst, data)
    ...
    if not shark then
        local tigersharker = GetWorld().components.tigersharker -- TODO
    ...
    --]]
    if shark then 
	    shark.Transform:SetPosition(x,y,z)
		shark.sg:GoToState("fallwarn")
...
local function kittenfn()
    ...
    -- MakePoisonableCharacter(inst)        -- 毒性组件，以后再说
    ...
    inst.components.locomotor.walkspeed = TUNING.SHARKITTEN_SPEED_WALK  -- 4
    inst.components.locomotor.runspeed = TUNING.SHARKITTEN_SPEED_RUN    -- 5
