...
local spreadFire = GetLockConfig("spread_fire")
--防止火焰蔓延
local portal_clear = GetLockConfig("portal_clear")
--小偷包保鲜
local krampus_sack_ice = GetLockConfig("krampus_sack_ice")
--远古犀牛刷新时间,如果小于0则不刷新
local minotaur_regenerate = GetLockConfig("minotaur_regenerate")
--蜂后重生时间
local respawn_beeqeen = GetLockConfig("respawn_beeqeen")
--蟾蜍重生时间
local respawn_toadstool = GetLockConfig("respawn_toadstool")
--中庭冷却时间
local respawn_atrium = GetLockConfig("respawn_atrium")
--远古犀牛是否允许拆家
local minotaur_destroy = GetLockConfig("minotaur_destroy")
--犬牙陷阱攻击无权限玩家
local trap_teeth_player = GetLockConfig("trap_teeth_player")
--眼球塔攻击无权限玩家
local eyeturret_player = GetLockConfig("eyeturret_player")
...
if IsServer then
	...
	if respawn_beeqeen >1 then
    _G.TUNING.BEEQUEEN_RESPAWN_TIME = 30 * 16 * respawn_beeqeen       --蜂后重生时间
	end
	if respawn_toadstool >1 then
    _G.TUNING.TOADSTOOL_RESPAWN_TIME = 30 * 16 * respawn_toadstool    --蟾蜍重生时间
	end
	if respawn_atrium >1 then
    _G.TUNING.ATRIUM_GATE_COOLDOWN = 30 * 16 * respawn_atrium         --中庭冷却时间
	end
